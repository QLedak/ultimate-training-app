...
2. **Phase structure** — off-season, pre-season, in-season, each with a distinct emphasis:
...
**Current state:** The library has grown to 238 exercises across every movement pattern, with a "Priority Tier" column flagging a curated Core 50 (the working set most programs should draw from first) against the wider Extended Library. Every row now has a real, written coach cue in the voice established in Section 6 — treat these as a strong first draft, not final; they should be reviewed and rewritten wherever they don't sound exactly right.
